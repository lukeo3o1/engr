...
| `heartbeat.sh` | `PostToolUse` on `Bash\|Edit\|Write` | Every `HEARTBEAT_EVERY` tool calls (15 by default) without a write to engr, and straight after a `git commit`, adds a one-line reminder to record what finished, settled or came up. Writes nothing. |
...
All three need `engr` and `git` on `PATH`, and do nothing in a repository without
`.engr`.
