...
rm -rf /audit/r8/scratch/fresh && mkdir -p /audit/r8/scratch/fresh && cd /audit/r8/scratch/fresh
...
diff .engr/.gitignore /audit/r8/scratch/fresh/.engr/.gitignore && echo "IDENTICAL"
echo "--- every migrated Event's admitted.at ---"
cat .engr/eventstore/objects/*.jsonl | sed -n 's/.*"admitted":{"at":"\([^"]*\)".*/\1/p'
echo "pre-confirm  wallclock: $(cat /audit/r8/evidence/pre-confirm-wallclock.txt)"
echo "post-confirm wallclock: $(cat /audit/r8/evidence/post-confirm-wallclock.txt)"
