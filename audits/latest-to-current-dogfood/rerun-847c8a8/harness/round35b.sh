...
echo
echo "===== the binaries this ran =====" 
for b in current prev; do
  printf '  engr-%-8s two-crash-windows sentence: ' "$b"
  /audit/bin/engr-$b protocol 2>/dev/null | grep -c 'There are two crash windows' || true
done
