...
timeout -s KILL 0.870 $E --root wi/w confirm "CONFIRM $CODE" >/dev/null 2>&1
...
echo "-- and what a fresh migrate does with a staged plan --"
__out=$($E --root wi/w migrate 2>&1); __rc=$?
printf '%s\n' "$__out" | tail -2; echo "migrate exit=$__rc"
