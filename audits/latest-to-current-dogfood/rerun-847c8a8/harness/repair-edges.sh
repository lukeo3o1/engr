...
__out=$($C --root re2 candidate "$c" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -6; echo "  exit=$__rc"
echo "  -- and whether confirming it is still allowed --"
__out=$($C --root re2 confirm "CONFIRM $c" 2>&1); __rc=$?
...
echo "  -- and what the pending code renders now --"
__out=$($C --root re3 candidate "$c3" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -6; echo "  exit=$__rc"
echo "  -- and whether confirming it is still allowed --"
__out=$($C --root re3 confirm "CONFIRM $c3" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -2; echo "  exit=$__rc"
