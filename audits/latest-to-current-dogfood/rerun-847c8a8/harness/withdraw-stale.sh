...
__out=$($E --root ws/w confirm "CONFIRM $CODE" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -2; echo "confirm exit=$__rc"
echo "-- and preparing again --"
__out=$($E --root ws/w migrate 2>&1); __rc=$?
printf '%s\n' "$__out" | tail -3; echo "migrate exit=$__rc"
