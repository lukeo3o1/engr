...
found=""
for ms in 850 860 870 880 855 865 875 885; do
...
printf '%s\n' "$out" | tail -1
__out=$($OLD --root ss2/w confirm "CONFIRM $c" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -2; echo "exit=$__rc"
...
out=$($CUR --root ss2/w confirm "CONFIRM $CODE" 2>&1); code=$?
printf "%s
" "$out" | head -3; echo "exit=$code"

echo
echo "===== is the admitted work in the migrated record? ====="
if [ "$code" -ne 0 ] && [ -d ss2/w/.engr/local/migration/destination ]; then
  echo "REFUSED: nothing was published, so the work is still where the released tool left it"
  echo "copies of that wording still in the predecessor record: $(grep -c "Admitted with the released tool" ss2/w/.engr/objects/01a05e55-74eb-7370-b771-0008bd71d149.json)"
elif $CUR --root ss2/w show 01a05e55-74 2>&1 | grep -q "Admitted with the released tool"; then
