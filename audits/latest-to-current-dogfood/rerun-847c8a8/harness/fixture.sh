...
set -u
cd /audit/r8/project
sh harness/inventory.sh . > /audit/r8/evidence/inventory-pre-migration.txt
echo "--- restored predecessor inventory ---"
cat /audit/r8/evidence/inventory-pre-migration.txt
echo "--- against the committed audit's own record ---"
if diff -u /audit/r8/evidence/inventory-pre-migration-committed.txt \
         /audit/r8/evidence/inventory-pre-migration.txt; then
