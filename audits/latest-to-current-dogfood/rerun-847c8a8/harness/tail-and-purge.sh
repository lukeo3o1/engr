...
echo "stored sections: $(grep -o '"id":[0-9][0-9]*' "tp/.engr/objects/$EV.json" | wc -l), stream events: $(wc -l < tp/.engr/eventstore/objects/$EV.jsonl)"
...
__out=$($E --root tp verify $EV 2>&1); __rc=$?
printf '%s\n' "$__out" | head -6; echo "exit=$__rc"
...
__out=$($E --root tpp migrate 2>&1); __rc=$?
printf '%s\n' "$__out" | tail -2; echo "exit=$__rc"
sed -n '1,2p;4p' "tpg/$f" > /tmp/g && mv /tmp/g "tpg/$f"
echo "gap made, $(wc -l < tpg/$f) left; migrating:"
__out=$($E --root tpg migrate 2>&1); __rc=$?
