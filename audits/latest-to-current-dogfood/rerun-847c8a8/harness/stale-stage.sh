...
timeout -s KILL 0.870 $CUR --root ss/w confirm "CONFIRM $CODE" >/dev/null 2>&1
...
__out=$($OLD --root ss/w confirm "CONFIRM $c" 2>&1); __rc=$?
...
__out=$($CUR --root ss/w confirm "CONFIRM $CODE" 2>&1); __rc=$?
