...
__out=$($E --root sm/a confirm "CONFIRM $CODE" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -3; echo "confirm exit=$__rc"
...
__out=$($E --root sm/b confirm "CONFIRM $CODE" 2>&1); __rc=$?
printf '%s\n' "$__out" | head -3; echo "confirm exit=$__rc"
...
__out=$($E --root sm/b verify 2>&1); __rc=$?
printf '%s\n' "$__out" | head -4; echo "verify exit=$__rc"
