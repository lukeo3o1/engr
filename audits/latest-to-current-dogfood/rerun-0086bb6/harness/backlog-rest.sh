...
B=${1:?usage: backlog-rest.sh <backlog-id> <object-ref>}
OBJ=${2:?object ref}
...
$E backlog subjects $B --section 1 --subject "$OBJ" --expect "$(sec_token 1)"; echo "exit=$?"
echo
echo "===== produced: record durable knowledge, without resolving the point ====="
$E backlog produced $B --section 2 --target "$OBJ" --expect "$(sec_token 2)"; echo "exit=$?"
