...
set -u
cd /audit/r6/project
sh harness/inventory.sh . > /audit/r6/evidence/inventory-pre-migration.txt
echo "--- restored predecessor inventory ---"
cat /audit/r6/evidence/inventory-pre-migration.txt
echo "--- against the committed audit's own record ---"
if diff -u /audit/r6/evidence/inventory-pre-migration-committed.txt \
         /audit/r6/evidence/inventory-pre-migration.txt; then
