...
& "$A\s7.ps1" -Script crash-sweep.sh -Out "evidence\crash-sweep.txt" @instants | Out-Null
Write-Output "sweep done"
& "$A\s7.ps1" -Script crash-resume.sh -Out "evidence\crash-resume.txt" @instants | Out-Null
Write-Output "resume done"
& "$A\s7.ps1" -Script crash-objects-identical.sh -Out "evidence\crash-objects-identical.txt" | Out-Null
