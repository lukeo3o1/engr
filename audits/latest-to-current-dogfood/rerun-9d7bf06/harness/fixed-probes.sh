...
printf 'round 33 sentence, fixed build: '
$F protocol 2>/dev/null | grep -c 'a repair that offers a confirmation code'
...
for pair in "st-missing absent" "st-divergent divergent" "st-tampered tampered"; do
  set -- $pair
  ws=$1; what=$2
  for bin in "$F fixed" "$C 9d7bf06"; do
    set -- $bin
    b=$1; who=$2
