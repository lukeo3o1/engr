...
set -u
cd /audit/r7/project
sh harness/inventory.sh . > /audit/r7/evidence/inventory-pre-migration.txt
echo "--- restored predecessor inventory ---"
cat /audit/r7/evidence/inventory-pre-migration.txt
echo "--- against the committed audit's own record ---"
if diff -u /audit/r7/evidence/inventory-pre-migration-committed.txt \
         /audit/r7/evidence/inventory-pre-migration.txt; then
